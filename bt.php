...
class Bt{
	...
	private $BT_KEY = "";  	//接口密钥
  	...
  	public function cshzh($bt_panel = null,$bt_key = null){
  	    $this->BT_PANEL="";
  	    $this->BT_KEY="";
  		$this->BT_PANEL = $bt_panel;
  		 $this->BT_KEY = $bt_key;
	...
	//开始防火墙操作
		/**
	 * 设置防火墙黑名单
	 * @param [type] $ksip      开始IP
	 * @param [type] $jsip      结束IP
	 */

	public function fhqhmd($ksip,$jsip){
		$url = $this->BT_PANEL.$this->config("fhqhmd");
		$p_data = $this->GetKeyData();
		$p_data['start_ip'] = $ksip;
		$p_data['end_ip'] = $jsip;
		$result = $this->HttpPostCookie($url,$p_data);
		$data = json_decode($result,true);
		return $data;
	...
	/*
		 * 设置防火墙黑名单
	 * @param [type] $ip      开始IP
	   @param [type] $types  方式 默认拒绝 drop
	 * @param [type] $beizhu  备注
	 */

	public function xtfhqhmd($ip,$types="drop",$beizhu){
		$url = $this->BT_PANEL.$this->config("xtfhq");
		$p_data = $this->GetKeyData();
		$p_data['address'] = $ip;
		$p_data['types'] = $types;
		$p_data['brief'] = $beizhu;
	...
	private function HttpPostCookie($url, $data,$timeout = 60)
	{
		...
		$cookie_file='./cookis8520/'.md5($this->BT_PANEL).'.cookie';
